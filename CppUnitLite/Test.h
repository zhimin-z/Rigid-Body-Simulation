...
{ long actualTemp = long(actual); \
  long expectedTemp = long(expected); \
...
#define FLOATS_EQUAL_THRESHOLD(expected,actual,threshold)\
{ float actualTemp = actual; \
	float expectedTemp = expected; \
	if (fabs ((expectedTemp)-(actualTemp)) > threshold) \
{ result_.addFailure (Failure (name_, __FILE__, __LINE__, \
	StringFrom((float)expectedTemp), StringFrom((float)actualTemp))); return; } }

#define FLOATS_EQUAL(expected,actual) FLOATS_EQUAL_THRESHOLD(expected,actual,1e-3f);

#define FAIL(text) \
{ result_.addFailure (Failure (name_, __FILE__, __LINE__,(text))); return; }

#define ALLOW_TEST_ACCESS(testName, testGroup)\
